...
function query(filterBy = null) {
  ...
  const filteredBooks = _getFilteredCars(books, filterBy);
  return Promise.resolve(filteredBooks);
}

function _getFilteredCars(books, filterBy) {
  let { name, maxPrice, minPrice } = filterBy
  minPrice = minPrice ? minPrice : 0
  maxPrice = maxPrice ? maxPrice : Infinity
  return books.filter(book => {
      return book.title.toUpperCase().includes(name.toUpperCase()) && book.listPrice.amount >= minPrice && book.listPrice.amount <= maxPrice
  })
}
...
function _createBooks() {
  let books = _loadCarsFromStorage();
...
function _saveCarsToStorage(books) {
